...
This document describes the frontend/orchestration workstream only. The branch does not add Supabase schema, migrations, RLS, service-role logic, storage policy, or a second production persistence layer. The current local adapter remains a demo contract until the Supabase foundation is rebased.
...
| `download_allowed` | Server authorizes asset ID | Stream the private demo asset or, after backend rebase, use the private storage contract. |
...
`Supabase foundation → rebase this branch → replace demo adapters with production contracts → rerun the complete funnel matrix → final review → merge`.
